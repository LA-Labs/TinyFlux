...
class SubscriptionWrapper<State>: Hashable {
    ...
    init<T>(
        ...
        if let transformedSubscription = transformedSubscription {
            transformedSubscription.observer = { [unowned self] oldState, newState in
                self.subscriber?._newState(newValue: newState, oldValue: oldState)
            }
        // If we haven't received a transformed subscription, we forward all values
        // from the original subscription.
        } else {
            originalSubscription.observer = { [unowned self] oldState, newState in
                self.subscriber?._newState(newValue: newState, oldValue: oldState)
            }
        }
    }
    ...
    static func == (left: SubscriptionWrapper<State>, right: SubscriptionWrapper<State>) -> Bool {
